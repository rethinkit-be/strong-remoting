...
const PhaseList = require('loopback-phase').PhaseList;
...
RemoteObjects.prototype.registerPhaseHandler = function(phaseName,
  methodNameWildcard,
  handler) {
  const pattern = methodNameWildcard.replace(/[.+?^${}()|[\]\\]/g, '\\$&')
...
RemoteObjects.prototype.execHooks = function(when, method, scope, ctx, next) {
  ...
  if (typeof handler === 'function') {
    ...
    let l = arguments.length; // eslint-disable-line one-var
    let i; // eslint-disable-line one-var
  ...
  function execStack(err) {
    ...
    if (cur) {
      try {
        const result = cur.call(scope, ctx, execStack, method);
